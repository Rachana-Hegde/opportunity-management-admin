...
function checkSessionAndRestore() {
    ...
    .then(data => {
        if (data.logged_in) {
            showDashboard(data.email); // restore UI
        }
    });
}
...
function loadOpportunities() {
    ...
    .then(data => {
        ...
        data.forEach(op => {
            ...
            console.log("Skills from backend:", op.skills);

            const formattedDate = new Date(op.start_date).toLocaleString('en-US', {
                month: 'short',
                year: 'numeric'
            });
            const formattedDuration = op.duration.includes('month')
            ? op.duration
            : op.duration + ' months';
...
function showDashboard(email) {
    ...
    if (savedPage) {
        document.querySelector(`.nav-item[data-page="${savedPage}"]`)?.click();
    }
    loadOpportunities();
}
...
function handleLogout() {
    ...
    .then(() => {
        localStorage.removeItem('activePage'); // clear saved page
...
function openEditModal(op) {
    openOpportunityModal(); // reuse existing modal
    ...
    // 🔥 store ID (important)
    document.getElementById('opportunityForm').dataset.editId = op.id;
}
...
const url = editId ? `/opportunities/${editId}` : '/opportunities';
...
.then(() => {
    ...
    this.reset();
    this.dataset.editId = '';   // 🔥 IMPORTANT (exit edit mode)
...
.then(result => {
    if (result.status === 200) {
        ...
        showError('loginPasswordErr', result.body.error || 'Login failed');
        shakeForm('loginForm');
    }
})
.catch(() => {
    showError('loginPasswordErr', 'Server error');
});
...
.then(result => {
    if (result.status === 201) {
        ...
        showError('signupEmailErr', result.body.error || 'Signup failed');
